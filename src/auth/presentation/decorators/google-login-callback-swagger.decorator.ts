...
export function ApiGoogleLoginCallback() {
  return applyDecorators(
    ApiOperation({ summary: '구글 소셜 로그인 콜백 처리' }),
    ApiQuery({
      name: 'code',
      required: true,
      type: String,
    ...
    createSwaggerUnauthorized([
      ...
      },
      {
        ...
        message: 'Google 계정 이메일 정보가 없어 로그인할 수 없습니다.',
      },
      {
        description: '이름 정보 없음',
        message: 'Google 계정 이름 정보가 없어 로그인할 수 없습니다.',
      },
    ]),
    ...createSwaggerServerErrors(),
  );
